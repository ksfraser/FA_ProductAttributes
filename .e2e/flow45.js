...
(async () => {
  ...
  page.on('response', async (resp)=>{
    if(resp.request().method()==='POST' && resp.url().includes('items.php')){
      ...
      postTexts.push({status:resp.status(), marker: m?m[0]:'(no VAR_TAB_POST)'});
    }
  });
  ...
  const selInner = ()=> page.evaluate(()=>document.getElementById('_tabs_div').innerHTML.length);
  await nav('/');
  ...
  const beforeLen = await selInner();
  postTexts=[]; navCount=0;
  await page.locator('input[name="generate_combos"]').click();
  await pause(5000); await page.waitForLoadState('networkidle');
  const afterLen = await selInner();
  console.log('GEN innerHTML before='+beforeLen+' after='+afterLen+' identical='+(beforeLen===afterLen));
  console.log('GEN_POSTS', JSON.stringify(postTexts,null,1));
