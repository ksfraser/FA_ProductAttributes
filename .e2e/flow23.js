...
(async () => {
  ...
  const posts=[];
  page.on('request', r=>{ if(r.method()==='POST' && r.url().includes('items.php')){ const p=r.postData()||''; if(p.includes('generate')||p.includes('assign')) posts.push({u:r.url().slice(-30), b:p.slice(0,300)}); } });
  ...
  await page.fill('input[name="description"]','E2E MM '+pk);
  ...
  await activateVar(pid);
  posts.length=0;
  console.log('generate_combos btn count:', await page.locator('input[name="generate_combos"],button[name="generate_combos"]').count());
  await page.click('input[name="generate_combos"],button[name="generate_combos"]');
  await page.waitForTimeout(3000);
  console.log('POSTs after GC click:', posts.map(p=>'URL='+p.u+' BODY='+p.b));
  await browser.close();
