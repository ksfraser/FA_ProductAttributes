...
(async () => {
  ...
  await page.fill('input[name="description"]','E2E VV '+pk);
  ...
  // click generate_combos and read the CURRENT DOM body (no re-navigation)
  await page.click('input[name="generate_combos"]');
  await page.waitForTimeout(4000);
  let body = await page.locator('body').innerText();
  console.log('URL after GC click:', page.url());
  // print any notification-ish lines
  const lines = body.split('\n').map(s=>s.trim()).filter(s=>/Combination|No categor|No values|Invalid|Cannot|up to date|saved/i.test(s));
  console.log('GC DOM notification lines:', JSON.stringify(lines));
  const idx = body.indexOf('Assigned Categories');
  console.log('GC panel context:', JSON.stringify(body.slice(idx, idx+300)));

  // click generate_child and read DOM
  await page.click('input[name="generate_child"]');
  await page.waitForTimeout(4000);
  body = await page.locator('body').innerText();
  const lines2 = body.split('\n').map(s=>s.trim()).filter(s=>/Generate Child|No combination|instantiated|Existing Variations|E2E-VV-/i.test(s));
  console.log('GCh DOM notification lines:', JSON.stringify(lines2.slice(0,10)));

  // children in dropdown
  await nav('/inventory/manage/items.php'); await page.waitForLoadState('networkidle');
  const opts = await page.evaluate(()=>{
    const sels=Array.from(document.querySelectorAll('select'));
    for(const s of sels){ const tt=s.innerText||''; if(tt.includes('E2E-VV-')) return tt; } return null;
  });
  const kids = opts? opts.split('\n').map(x=>x.trim()).filter(x=>x.includes('E2E-VV-')) : [];
