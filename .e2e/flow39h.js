...
(async () => {
  const browser = await chromium.launch({
    executablePath: EXE, headless: false, args: ['--no-sandbox','--disable-dev-shm-usage']
  });
  ...
  await page.fill('input[name="description"]','E2E AE '+pk);
  ...
  const gcMsg=panelGC.split('\n').map(s=>s.trim()).filter(s=>/Combination|No categor|No values|up to date|saved/i.test(s));
  console.log('panel msgs after GC:', JSON.stringify(gcMsg));
  ...
  const gchMsg=panelGCh.split('\n').map(s=>s.trim()).filter(s=>/Generate Child|No combination|instantiat|Existing Variations|E2E-AE-/i.test(s));
  ...
  const opts = await page.evaluate(()=>{ const sels=Array.from(document.querySelectorAll('select')); for(const s of sels){const t=s.innerText||''; if(t.includes('E2E-AE-')) return t;} return null;});
  const kids=opts?opts.split('\n').map(x=>x.trim()).filter(x=>x.startsWith(pid+'-')):[];
  console.log('DROPDOWN children:', kids.length, JSON.stringify(kids.slice(0,12)));
  await browser.close();
